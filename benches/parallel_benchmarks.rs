...
fn create_parallel_engine_and_kb(
    rule_count: usize,
    user_count: usize,
    threads: usize,
) -> (ParallelRuleEngine, KnowledgeBase, Facts) {
...
fn bench_small_sequential_vs_parallel(c: &mut Criterion) {
    ...
    group.bench_function("parallel_2threads_10rules", |b| {
        b.iter(|| {
            black_box(
                par_engine_2
                    .execute_parallel(&kb_2, &par_facts_2, false)
                    .unwrap(),
            );
        })
    });
    ...
    group.bench_function("parallel_4threads_10rules", |b| {
        b.iter(|| {
            black_box(
                par_engine_4
                    .execute_parallel(&kb_4, &par_facts_4, false)
                    .unwrap(),
            );
        })
    });
...
fn bench_thread_scaling(c: &mut Criterion) {
    ...
    for threads in [1, 2, 4, 8, 12, 16].iter() {
        let (par_engine, kb, par_facts) =
            create_parallel_engine_and_kb(rule_count, user_count, *threads);
        group.throughput(Throughput::Elements(rule_count as u64));
        group.bench_with_input(
